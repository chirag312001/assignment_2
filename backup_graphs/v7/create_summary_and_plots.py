#!/usr/bin/env python3
"""
Improved plotting: grouped bars annotated with values; ROB legend preserved.
"""
import os
import pandas as pd
...
def main(results_dir, out_dir):
    ...
    for wl in df["workload"].unique():
        ...
        ax.bar(np.arange(len(sub)), sub["ipc"], color=colors)
    ...
    # ---------- Grouped bar charts by ROB for each workload & IQ (annotated) ----------
    for wl in df["workload"].unique():
        ...
        for iq in iq_values:
            ...
            # choose color per ROB and draw bars, collect rects for labeling
            rects_by_rob = {}
            for j, rob in enumerate(pivot.columns):
                vals = pivot[rob].values
                rects = ax.bar(x + j*width, vals, width=width, label=f"ROB{rob}", color=COLORMAP(j % COLORMAP.N))
                rects_by_rob[rob] = rects

                # add numeric labels on each bar (top)
                for k, rect in enumerate(rects):
                    h = rect.get_height()
                    if not np.isnan(h):
                        ax.text(rect.get_x() + rect.get_width()/2.0, h, f"{h:.3f}", ha="center", va="bottom", fontsize=8, rotation=0)

            # ticks and labels (short)
