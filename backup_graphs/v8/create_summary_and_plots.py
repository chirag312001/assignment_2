#!/usr/bin/env python3
"""
Analysis + plotting for gem5 stats:
- Annotated grouped bars with ROB and IPC values (multi-line labels)
- Auto-zoom y-axis to highlight small IPC differences
- Legends retained for line/scatter plots
"""
import os
import pandas as pd
...
def auto_ylim(values, lower_pad_factor=0.02, upper_pad_factor=0.01, min_span=0.001):
    """Return (ymin, ymax) tightly around values with small margins."""
    ...
    # ensure ymin is not negative if values are positive and large numbers
...
def main(results_dir, out_dir):
    ...
    for wl in df["workload"].unique():
        ...
        colors = [plt.colormaps.get_cmap("Set2")(i % 8) for i in range(len(sub))]
        x = np.arange(len(sub))
        ...
        # auto zoom around IPC values
        ymin, ymax = auto_ylim(sub["ipc"].values)
        ax.set_ylim(ymin, ymax)

        # annotate each bar with predictor (short), IPC value and optionally IQ/ROB info
        for i, rect in enumerate(bars):
            h = rect.get_height()
            if not np.isnan(h):
                # example multi-line label: ROB128\n1.602
                rob_val = sub.iloc[i]["ROB"]
                iq_val = sub.iloc[i]["IQEntries"]
    ...
    # ---------- Grouped bar charts by ROB for each workload & IQ (annotated with ROB and value) ----------
    for wl in df["workload"].unique():
        ...
        for iq in iq_values:
            ...
            # draw bars per ROB and annotate each with ROB + ipc value
            for j, rob in enumerate(pivot.columns):
                vals = pivot[rob].values
                rects = ax.bar(x + j*width, vals, width=width, label=f"ROB{rob}", color=plt.colormaps.get_cmap("Set2")(j % 8))
                for k, rect in enumerate(rects):
                    h = rect.get_height()
                    if not np.isnan(h):
                        # annotate with ROB and IPC (multi-line)
                        label = f"ROB{rob}\n{h:.3f}"
                        ax.text(rect.get_x() + rect.get_width()/2.0, h, label, ha="center", va="bottom", fontsize=8)

            # ticks and labels (short predictor names)
            ax.set_xticks(x + width*(len(pivot.columns)-1)/2)
            ax.set_xticklabels([short_label(p) for p in pivot.index], rotation=35, ha="right")

            # auto-zoom y-axis using all bar heights
            ...
            # ROB legend for color reference (kept for quick glance)
    ...
    for wl in df["workload"].unique():
        ...
        if df_w["ROB"].notnull().any():
            ...
            for idx, pred in enumerate(preds):
                ...
                color = plt.colormaps.get_cmap("tab10")(idx % 10)
